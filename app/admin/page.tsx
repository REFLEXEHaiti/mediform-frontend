// app/admin/page.tsx — MediForm Haiti
...
function ContenuAdmin() {
  ...
  const [formSponsor,  setFormSponsor]  = useState({ nom:'', siteWeb:'', typeContrat:'OR', couleur:'#1B6B45', logoUrl:'' });
  ...
  const ajouterSponsor = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setFormSponsor({ nom:'', siteWeb:'', typeContrat:'OR', couleur:'#1B6B45', logoUrl:'' });
      setPreview('');
    } catch { toast.error('Erreur'); }
  };
                ...
                <tbody>
                  {utilisateurs.map(u => (
                    <tr key={u.id} style={{ borderTop: '1px solid #F1F5F9' }}>
                      ...
                      <td style={{ padding: '12px 16px' }}>
                        <span style={{ background: u.abonnement?.statut === 'ACTIF' ? '#E8F5ED' : '#F1F5F9', color: u.abonnement?.statut === 'ACTIF' ? VERT : '#94A3B8', fontSize: 11, fontWeight: 700, padding: '3px 8px', borderRadius: 100, fontFamily: "'Inter',sans-serif" }}>
                ...
                <div key={f.id} style={{ padding: '14px 20px', borderTop: i > 0 ? '1px solid #F1F5F9' : 'none', display: 'flex', alignItems: 'center', gap: 16 }}>
                  ...
                  <div style={{ display: 'flex', gap: 10, alignItems: 'center', flexShrink: 0 }}>
                    <button onClick={() => toggleFormation(f.id, f.publie)}
                      style={{ padding: '6px 14px', background: f.publie ? '#E8F5ED' : '#F1F5F9', color: f.publie ? VERT : '#64748B', border: 'none', borderRadius: 8, fontFamily: "'Inter',sans-serif", fontSize: 12, fontWeight: 700, cursor: 'pointer' }}>
              ...
              {abonnements.length === 0 ? (
                ...
                <div key={ab.id} style={{ padding: '14px 20px', borderTop: '1px solid #F1F5F9', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 10 }}>
                  ...
                  <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
                    <span style={{ fontSize: 11, fontWeight: 700, padding: '4px 10px', borderRadius: 100, background: ab.statut === 'ACTIF' ? '#E8F5ED' : '#FEF2F2', color: ab.statut === 'ACTIF' ? VERT : '#DC2626', fontFamily: "'Inter',sans-serif" }}>
