...
const FORMATION_DEMO = {
  id: 'demo', titre: 'Soins infirmiers fondamentaux', description: 'Les bases des soins infirmiers : hygiène, administration des médicaments, surveillance des constantes vitales.',
  niveau: 'DEBUTANT', categorie: 'Soins infirmiers', publie: true, gratuit: true,
  lecons: [
    { id: 'l1', titre: 'Hygiène des mains et asepsie', dureeMin: 20, quiz: true, quizNbQuestions: 5 },
    { id: 'l2', titre: 'Surveillance des constantes vitales', dureeMin: 30, quiz: true, quizNbQuestions: 8 },
  ...
  examens: [
    ...
    { id: 'ex2', titre: 'Examen final — Soins infirmiers fondamentaux', type: 'EXAMEN', nbQuestions: 30, dureeMin: 60, apresLecon: 5, statut: 'VERROUILLE', dateDebut: '2026-05-01', dateFin: '2026-05-07', formateurNom: 'Dr. Marie Théodore' },
  ],
...
function MiniQuiz({ quiz, onTerminer }: { quiz: QuizEmbed; onTerminer: (score: number) => void }) {
  ...
  return (
    <div style={{ background: '#F4FAF6', border: `2px solid ${VERT}`, borderRadius: 14, padding: '20px 24px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
        <div>
          ...
          <div style={{ fontFamily: "'Helvetica Neue',Arial,sans-serif", fontSize: 12, color: '#4A7A5A' }}>Question {idx + 1} / {questions.length}</div>
        </div>
        <div style={{ height: 4, width: 80, background: '#D1E7D9', borderRadius: 2, overflow: 'hidden' }}>
          <div style={{ height: '100%', background: VERT, width: `${((idx + (valide ? 1 : 0)) / questions.length) * 100}%` }} />
        </div>
      </div>

      <p style={{ fontFamily: 'Georgia,serif', fontSize: 16, color: '#0D2818', lineHeight: 1.55, margin: '0 0 16px' }}>{q.texte}</p>
      ...
      {valide && (
        <div style={{ background: '#F0FDF4', border: '1px solid #BBF7D0', borderRadius: 8, padding: '10px 14px', marginBottom: 12 }}>
          <div style={{ fontFamily: "'Helvetica Neue',Arial,sans-serif", fontSize: 12, fontWeight: 700, color: '#166534', marginBottom: 3 }}>💡 Explication</div>
          <p style={{ fontFamily: "'Helvetica Neue',Arial,sans-serif", fontSize: 12, color: '#166534', lineHeight: 1.5, margin: 0 }}>{q.explication}</p>
        </div>
      )}

      {!valide ? (
        <button onClick={validerEtSuivre} disabled={reponseChoisie === null}
          style={{ width: '100%', padding: '12px', background: reponseChoisie !== null ? VERT : '#D1E7D9', color: reponseChoisie !== null ? 'white' : '#6B9E7A', border: 'none', borderRadius: 8, fontFamily: "'Helvetica Neue',Arial,sans-serif", fontWeight: 700, fontSize: 14, cursor: reponseChoisie !== null ? 'pointer' : 'default' }}>
          Valider →
        </button>
        ...
        <div style={{ textAlign: 'center', padding: '8px 0', fontFamily: "'Helvetica Neue',Arial,sans-serif", fontSize: 13, color: '#4A7A5A' }}>
...
export default function PageFormationDetail() {
  ...
  if (chargement) return (
    <div style={{ minHeight: '60vh', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ width: 32, height: 32, border: `3px solid #E8F5ED`, borderTopColor: VERT, borderRadius: '50%', animation: 'spin 0.8s linear infinite' }} />
  ...
  if (!formation) return (
    <div style={{ minHeight: '60vh', display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: 16 }}>
      ...
      <h2 style={{ fontFamily: 'Georgia,serif', fontSize: 20, fontWeight: 'normal', color: '#0D2818' }}>Formation introuvable</h2>
  ...
  return (
    <div style={{ background: '#F4FAF6', minHeight: '100vh' }}>
      ...
      {/* Hero */}
      <div style={{ background: `linear-gradient(135deg, #0D2818 0%, ${primaire} 100%)`, padding: 'clamp(32px,5vw,56px) clamp(20px,5vw,56px)' }}>
          ...
          <div style={{ background: 'white', borderRadius: 16, padding: '24px', minWidth: 220, boxShadow: '0 16px 48px rgba(0,0,0,0.25)', flexShrink: 0 }}>
            {dejaInscrit && progression ? (
              ...
              <>
                ...
                <div style={{ fontFamily: "'Helvetica Neue',Arial,sans-serif", fontSize: 11, color: '#6B9E7A', marginBottom: 16, lineHeight: 1.5 }}>
      ...
      <div style={{ maxWidth: 1000, margin: '0 auto', padding: 'clamp(24px,4vw,48px) clamp(20px,5vw,56px)' }}>
        ...
        {/* Onglets */}
        <div style={{ display: 'flex', gap: 4, marginBottom: 28, background: 'white', border: '1px solid #D1E7D9', borderRadius: 12, padding: 4, width: 'fit-content' }}>
          {[{ id: 'programme', label: '📚 Programme' }, { id: 'examens', label: `📋 Quiz & Examens (${examens.length})` }].map(o => (
            <button key={o.id} onClick={() => setOnglet(o.id as any)}
              style={{ padding: '10px 20px', borderRadius: 9, border: 'none', background: onglet === o.id ? primaire : 'transparent', color: onglet === o.id ? 'white' : '#4A7A5A', fontFamily: "'Helvetica Neue',Arial,sans-serif", fontWeight: onglet === o.id ? 700 : 400, fontSize: 13, cursor: 'pointer', transition: 'all 0.15s' }}>
              {o.label}
            </button>
          ))}
        </div>
        ...
        {onglet === 'programme' && (
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 300px', gap: 24 }}>
            <div>
              {/* Objectifs */}
              <div style={{ background: 'white', border: '1px solid #D1E7D9', borderRadius: 12, padding: '20px 24px', marginBottom: 20 }}>
                <h2 style={{ fontFamily: 'Georgia,serif', fontSize: 18, fontWeight: 'normal', color: '#0D2818', margin: '0 0 14px' }}>Ce que vous apprendrez</h2>
                {['Maîtriser les gestes infirmiers fondamentaux', 'Surveiller et interpréter les constantes vitales', 'Administrer les médicaments en sécurité', 'Prévenir les infections en milieu de soins'].map((item, i) => (
              ...
              {formation.lecons?.length > 0 && (
                <div style={{ background: 'white', border: '1px solid #D1E7D9', borderRadius: 12, overflow: 'hidden' }}>
                  <div style={{ padding: '16px 24px', borderBottom: '1px solid #E8F5ED', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <h2 style={{ fontFamily: 'Georgia,serif', fontSize: 18, fontWeight: 'normal', color: '#0D2818', margin: 0 }}>
                    ...
                    return (
                      <div key={l.id ?? i}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '14px 24px', borderBottom: '1px solid #F0FAF4' }}>
                          <div style={{ width: 36, height: 36, borderRadius: '50%', background: termine ? `${primaire}15` : '#F0FAF4', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 14, color: termine ? primaire : '#6B9E7A', fontWeight: 700, flexShrink: 0 }}>
                            {termine ? '✓' : i + 1}
                          </div>
                          <div style={{ flex: 1 }}>
                            {accessible && dejaInscrit ? (
                              <Link href={`/formations/${id}/lecons/${l.id}`} style={{ fontFamily: "'Helvetica Neue',Arial,sans-serif", fontSize: 14, fontWeight: 600, color: '#0D2818', textDecoration: 'none', display: 'block', marginBottom: 2 }}>{l.titre}</Link>
                            ) : (
                              <p style={{ fontFamily: "'Helvetica Neue',Arial,sans-serif", fontSize: 14, fontWeight: 600, color: '#0D2818', margin: '0 0 2px' }}>{l.titre}</p>
                            )}
                            <div style={{ display: 'flex', gap: 12 }}>
                              <span style={{ fontFamily: "'Helvetica Neue',Arial,sans-serif", fontSize: 11, color: '#6B9E7A' }}>⏱ {l.dureeMin} min</span>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              <div style={{ background: 'white', border: '1px solid #D1E7D9', borderRadius: 12, padding: '18px' }}>
                <h3 style={{ fontFamily: 'Georgia,serif', fontSize: 15, fontWeight: 'normal', color: '#0D2818', margin: '0 0 12px' }}>Inclus dans la formation</h3>
              ...
              {examens.some(e => e.statut !== 'FERME') && (
                <div style={{ background: `${secondaire}08`, border: `1px solid ${secondaire}25`, borderRadius: 12, padding: '18px' }}>
                  <h3 style={{ fontFamily: 'Georgia,serif', fontSize: 15, fontWeight: 'normal', color: '#0D2818', margin: '0 0 12px' }}>📋 Examens planifiés</h3>
                  {examens.filter(e => e.statut !== 'FERME').map(ex => (
                    <div key={ex.id} style={{ marginBottom: 10, paddingBottom: 10, borderBottom: '1px solid #D1E7D9' }}>
                      <div style={{ fontFamily: "'Helvetica Neue',Arial,sans-serif", fontSize: 12, fontWeight: 700, color: '#0D2818', marginBottom: 4 }}>{ex.titre}</div>
                      {ex.dateDebut && (
                        <div style={{ fontFamily: "'Helvetica Neue',Arial,sans-serif", fontSize: 11, color: '#4A7A5A' }}>
        ...
        {onglet === 'examens' && (
          <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
              <div>
                <h2 style={{ fontFamily: 'Georgia,serif', fontSize: 22, color: '#0D2818', margin: '0 0 6px', fontWeight: 'normal' }}>Quiz & Examens</h2>
                <p style={{ fontFamily: "'Helvetica Neue',Arial,sans-serif", fontSize: 13, color: '#4A7A5A', margin: 0 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              {examens.map(ex => {
                ...
                return (
                  <div key={ex.id} style={{ background: 'white', border: `1px solid ${ouvert ? primaire + '40' : '#D1E7D9'}`, borderRadius: 14, padding: '20px 24px' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 16, flexWrap: 'wrap' }}>
                      <div style={{ flex: 1 }}>
                        ...
                        <h3 style={{ fontFamily: 'Georgia,serif', fontSize: 17, color: '#0D2818', margin: '0 0 8px', fontWeight: 'normal' }}>{ex.titre}</h3>
                        <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap' }}>
                          <span style={{ fontFamily: "'Helvetica Neue',Arial,sans-serif", fontSize: 12, color: '#4A7A5A' }}>❓ {ex.nbQuestions} questions</span>
                          <span style={{ fontFamily: "'Helvetica Neue',Arial,sans-serif", fontSize: 12, color: '#4A7A5A' }}>⏱ {ex.dureeMin} min</span>
                          {ex.formateurNom && <span style={{ fontFamily: "'Helvetica Neue',Arial,sans-serif", fontSize: 12, color: '#4A7A5A' }}>👨‍⚕️ {ex.formateurNom}</span>}
                          {ex.dateDebut && <span style={{ fontFamily: "'Helvetica Neue',Arial,sans-serif", fontSize: 12, color: '#4A7A5A' }}>📅 {new Date(ex.dateDebut).toLocaleDateString('fr-FR')} → {ex.dateFin ? new Date(ex.dateFin).toLocaleDateString('fr-FR') : '—'}</span>}
                          {ex.apresLecon && <span style={{ fontFamily: "'Helvetica Neue',Arial,sans-serif", fontSize: 12, color: '#4A7A5A' }}>🔓 Débloqué après leçon {ex.apresLecon}</span>}
                        </div>
                      </div>
                      <div style={{ flexShrink: 0, display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 8 }}>
                        {scoreExistant !== undefined && (
                          <div style={{ background: scoreExistant >= 60 ? '#DCFCE7' : '#FEF3C7', borderRadius: 8, padding: '6px 12px', textAlign: 'center' }}>
                            ...
                            <div style={{ fontFamily: "'Helvetica Neue',Arial,sans-serif", fontSize: 10, color: '#4A7A5A' }}>Dernier score</div>
                          </div>
                        )}
                        {ouvert && dejaInscrit ? (
                          ...
                          <button onClick={sInscrire}
                            style={{ padding: '10px 20px', background: '#F1F5F9', color: '#64748B', border: '1px solid #D1E7D9', borderRadius: 8, fontFamily: "'Helvetica Neue',Arial,sans-serif", fontWeight: 700, fontSize: 13, cursor: 'pointer' }}>
