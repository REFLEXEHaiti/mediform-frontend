// app/paiement/annule/page.tsx — MediForm Haiti
'use client';
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#FEF2F2', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '32px 16px' }}>
      <div style={{ background: 'white', borderRadius: 24, padding: 'clamp(32px,5vw,56px)', maxWidth: 480, width: '100%', textAlign: 'center', boxShadow: '0 24px 64px rgba(0,0,0,0.08)', border: '1px solid #FECACA' }}>
        ...
        <div style={{ marginTop: 24, background: '#F4FAF6', borderRadius: 10, padding: '14px 18px' }}>
          <p style={{ fontFamily: "'Helvetica Neue',Arial,sans-serif", fontSize: 13, color: '#4A7A5A', margin: 0, lineHeight: 1.6 }}>
            💡 <strong>Besoin d'aide ?</strong> Contactez-nous à{' '}
            <a href={`mailto:${config?.emailContact ?? 'contact@mediformhaiti.com'}`} style={{ color: primaire, textDecoration: 'none', fontWeight: 700 }}>
              {config?.emailContact ?? 'contact@mediformhaiti.com'}
            </a>
          </p>
        </div>
      </div>
