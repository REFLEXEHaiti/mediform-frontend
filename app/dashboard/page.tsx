// app/dashboard/page.tsx — MediForm Haiti
'use client';
...
export default function PageDashboard() {
  ...
  if (!pret) return <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '60vh' }}><div style={{ fontFamily: "'Helvetica Neue',Arial,sans-serif", color: '#4A7A5A', fontSize: 14 }}>Chargement…</div></div>;
  ...
  return (
    <div style={{ background: '#F4FAF6', minHeight: '100vh' }}>
      <div style={{ background: `linear-gradient(135deg, #0D2818 0%, ${primaire} 100%)`, padding: 'clamp(32px,5vw,56px) clamp(16px,4vw,40px)' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 16, flexWrap: 'wrap', marginBottom: 20 }}>
            ...
            <div>
              <h1 style={{ fontFamily: 'Georgia,serif', fontSize: 'clamp(18px,3vw,24px)', color: 'white', margin: '0 0 4px', fontWeight: 'normal' }}>Bonjour, Dr. {utilisateur?.prenom} !</h1>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: 24 }}>
            <div style={{ gridColumn: '1 / -1' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
                <h2 style={{ fontFamily: 'Georgia,serif', fontSize: 20, color: '#0D2818', margin: 0, fontWeight: 'normal' }}>Mes formations en cours</h2>
              ...
              {(!data?.inscriptions || data.inscriptions.length === 0) ? (
                <div style={{ background: 'white', borderRadius: 14, border: '1px solid #D1E7D9', padding: '32px', textAlign: 'center' }}>
                  <div style={{ fontSize: 40, marginBottom: 12 }}>🏥</div>
                  <p style={{ fontFamily: "'Helvetica Neue',Arial,sans-serif", fontSize: 14, color: '#4A7A5A', margin: '0 0 16px' }}>Aucune formation en cours.</p>
                  ...
                  {data.inscriptions.slice(0, 6).map((insc: any) => {
                    ...
                    return (
                      <Link key={insc.id} href={`/formations/${insc.coursId}`} style={{ textDecoration: 'none' }}>
                        <div style={{ background: 'white', borderRadius: 12, border: '1px solid #D1E7D9', padding: '14px 16px' }}>
                          <div style={{ fontFamily: "'Helvetica Neue',Arial,sans-serif", fontWeight: 700, fontSize: 14, color: '#0D2818', marginBottom: 8, lineHeight: 1.4 }}>{insc.cours?.titre}</div>
                          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6, fontFamily: "'Helvetica Neue',Arial,sans-serif", fontSize: 12, color: '#4A7A5A' }}>
                          ...
                          <div style={{ height: 4, background: '#E8F5ED', borderRadius: 2, overflow: 'hidden' }}>
            ...
            <div style={{ background: 'white', borderRadius: 14, border: '1px solid #D1E7D9', padding: '20px 24px' }}>
              <h2 style={{ fontFamily: 'Georgia,serif', fontSize: 18, color: '#0D2818', margin: '0 0 16px', fontWeight: 'normal' }}>Navigation rapide</h2>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {[
                  { href: '/formations',  label: '📚 Formations médicales',        bg: `${primaire}08`, color: primaire },
                  { href: '/tournois',    label: '🏥 Simulations cliniques',       bg: `${secondaire}10`, color: secondaire },
                  { href: '/lives',       label: '🎥 Conférences médicales',       bg: '#F0FDF4',  color: '#059669' },
                  { href: '/bibliotheque',label: '📖 Bibliothèque médicale',       bg: '#F5F3FF',  color: '#7C3AED' },
            ...
            <div style={{ background: 'white', borderRadius: 14, border: '1px solid #D1E7D9', padding: '20px 24px' }}>
              <h2 style={{ fontFamily: 'Georgia,serif', fontSize: 18, color: '#0D2818', margin: '0 0 16px', fontWeight: 'normal' }}>Certifications</h2>
              {(!data?.badges || data.badges.length === 0) ? (
                <p style={{ fontFamily: "'Helvetica Neue',Arial,sans-serif", fontSize: 13, color: '#6B9E7A', textAlign: 'center', padding: '20px 0' }}>Complétez des formations pour obtenir des certifications médicales.</p>
              ) : (
        ...
        {onglet === 'sponsors' && estAdmin && (
          <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
              <div>
                <h2 style={{ fontFamily: 'Georgia,serif', fontSize: 22, color: '#0D2818', margin: '0 0 6px', fontWeight: 'normal' }}>Partenaires santé</h2>
                <p style={{ fontFamily: "'Helvetica Neue',Arial,sans-serif", fontSize: 13, color: '#4A7A5A', margin: 0 }}>Apparaissent dans la bannière défilante sur toutes les pages.</p>
              </div>
            ...
            {sponsors.length === 0 ? (
              <div style={{ textAlign: 'center', padding: '48px 0', color: '#6B9E7A' }}><div style={{ fontSize: 48, marginBottom: 12 }}>🤝</div><p style={{ fontFamily: "'Helvetica Neue',Arial,sans-serif", fontSize: 14 }}>Aucun partenaire pour l'instant.</p></div>
            ) : (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                {sponsors.map(s => (
                  <div key={s.id} style={{ background: 'white', border: '1px solid #D1E7D9', borderRadius: 12, padding: '16px 20px', display: 'flex', alignItems: 'center', gap: 16 }}>
                    <div style={{ width: 60, height: 60, borderRadius: 10, background: '#F4FAF6', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, overflow: 'hidden', border: '1px solid #D1E7D9' }}>
                    ...
                    <div style={{ flex: 1 }}>
                      <div style={{ fontFamily: "'Helvetica Neue',Arial,sans-serif", fontWeight: 700, fontSize: 15, color: '#0D2818', marginBottom: 4 }}>{s.nom}</div>
          ...
          <div style={{ background: 'white', borderRadius: 16, padding: '28px 32px', width: '100%', maxWidth: 480, boxShadow: '0 24px 64px rgba(0,0,0,0.2)', maxHeight: '90vh', overflowY: 'auto' }}>
            ...
            <form onSubmit={ajouterSponsor} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              <div>
                ...
                <div style={{ background: '#F4FAF6', border: '1px solid #D1E7D9', borderRadius: 8, padding: '10px 14px', marginBottom: 8, fontFamily: "'Helvetica Neue',Arial,sans-serif", fontSize: 12, color: '#4A7A5A', lineHeight: 1.6 }}>
                ...
                <div onClick={() => logoRef.current?.click()} style={{ border: `2px dashed ${logoPreview ? primaire : '#D1E7D9'}`, borderRadius: 10, padding: 20, textAlign: 'center', cursor: 'pointer', background: logoPreview ? `${primaire}04` : '#F4FAF6' }}>
                  {logoPreview ? (
                    ...
                    <div><div style={{ fontSize: 32, marginBottom: 8 }}>📁</div><p style={{ fontFamily: "'Helvetica Neue',Arial,sans-serif", fontSize: 13, color: '#4A7A5A', margin: 0, fontWeight: 600 }}>Cliquer pour sélectionner le logo</p></div>
                  )}
                </div>
