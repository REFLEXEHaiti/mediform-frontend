// app/auth/reinitialiser-mot-de-passe/page.tsx — TechPro Haiti
'use client';
...
function ContenuPage() {
  ...
  const inp: React.CSSProperties = {
    width: '100%', padding: '13px 16px', border: `1.5px solid #CBD5E1`, borderRadius: 10,
    fontSize: 15, outline: 'none', fontFamily: "'Helvetica Neue',Arial,sans-serif",
    boxSizing: 'border-box' as const, color: '#0D1B2A', background: 'white',
  };

  return (
    <div style={{ minHeight: '100vh', background: '#F0F4FA', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '32px 16px' }}>
      <div style={{ width: '100%', maxWidth: 440 }}>

        <div style={{ textAlign: 'center', marginBottom: 28 }}>
          <div style={{ width: 60, height: 60, borderRadius: 16, background: `linear-gradient(135deg, #0D1B2A, ${primaire})`, display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 14px', color: 'white', fontFamily: 'Georgia,serif', fontWeight: 700, fontSize: 22 }}>MF</div>
          <h1 style={{ fontFamily: 'Georgia,serif', fontSize: 22, fontWeight: 'normal', color: '#0D1B2A', margin: 0 }}>TechPro Haiti</h1>
        </div>
        ...
        <div style={{ background: 'white', borderRadius: 20, padding: 'clamp(24px,4vw,36px)', border: '1px solid #CBD5E1', boxShadow: `0 8px 40px ${primaire}10` }}>
          ...
          {succes ? (
            <div style={{ textAlign: 'center' }}>
              ...
              <h2 style={{ fontFamily: 'Georgia,serif', fontSize: 22, fontWeight: 'normal', color: '#0D1B2A', margin: '0 0 12px' }}>Mot de passe mis à jour !</h2>
              <p style={{ fontFamily: "'Helvetica Neue',Arial,sans-serif", fontSize: 14, color: '#475569', lineHeight: 1.7, margin: '0 0 20px' }}>
            ...
            <>
              <div style={{ textAlign: 'center', marginBottom: 24 }}>
                ...
                <h2 style={{ fontFamily: 'Georgia,serif', fontSize: 22, fontWeight: 'normal', color: '#0D1B2A', margin: '0 0 8px' }}>Nouveau mot de passe</h2>
                <p style={{ fontFamily: "'Helvetica Neue',Arial,sans-serif", fontSize: 13, color: '#475569', lineHeight: 1.6, margin: 0 }}>
                  Choisissez un mot de passe sécurisé pour votre compte TechPro Haiti.
                </p>
              </div>
              ...
              <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                <div>
                  <label style={{ display: 'block', fontFamily: "'Helvetica Neue',Arial,sans-serif", fontSize: 13, fontWeight: 600, color: '#0D1B2A', marginBottom: 6 }}>
                    Nouveau mot de passe <span style={{ fontWeight: 400, color: '#64748B' }}>(min. 8 caractères)</span>
                  </label>
                  <div style={{ position: 'relative' }}>
                    ...
                    <button type="button" onClick={() => setVoirMDP(v => !v)} style={{ position: 'absolute', right: 14, top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', cursor: 'pointer', fontSize: 16, color: '#64748B' }}>
                      {voirMDP ? '🙈' : '👁'}
                    </button>
                  </div>
                </div>
                <div>
                  ...
                  <input type="password" value={confirmation} required onChange={e => setConfirmation(e.target.value)}
                    placeholder="••••••••" style={{ ...inp, borderColor: confirmation && motDePasse !== confirmation ? '#FCA5A5' : '#CBD5E1' }}
                    onFocus={e => { e.target.style.borderColor = primaire; }}
                    onBlur={e => { e.target.style.borderColor = confirmation && motDePasse !== confirmation ? '#FCA5A5' : '#CBD5E1'; }} />
                ...
                {motDePasse && (
                  <div>
                    <div style={{ display: 'flex', gap: 4, marginBottom: 4 }}>
                      {[1,2,3,4].map(n => (
                        <div key={n} style={{ flex: 1, height: 4, borderRadius: 2, background: motDePasse.length >= n * 2 ? (n <= 2 ? '#F59E0B' : '#16A34A') : '#CBD5E1' }} />
                      ))}
                    </div>
                    <span style={{ fontFamily: "'Helvetica Neue',Arial,sans-serif", fontSize: 11, color: '#64748B' }}>
                ...
                <button type="submit" disabled={chargement || !token}
                  style={{ width: '100%', padding: '15px', background: (chargement || !token) ? '#475569' : primaire, color: 'white', border: 'none', borderRadius: 12, fontFamily: "'Helvetica Neue',Arial,sans-serif", fontWeight: 700, fontSize: 16, cursor: (chargement || !token) ? 'not-allowed' : 'pointer' }}>
...
export default function PageReinitialiserMotDePasse() {
  return (
    <Suspense fallback={<div style={{ minHeight: '100vh', background: '#F0F4FA' }} />}>
      <ContenuPage />
    </Suspense>
  );
}
