// app/auth/connexion/page.tsx — MediForm Haiti
'use client';
...
export default function PageConnexion() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#F4FAF6', display: 'flex', flexDirection: 'column' }}>

      {/* Bannière sponsors */}
      {sponsors.length > 0 && (
        <div style={{ background: 'white', borderBottom: '1px solid #D1E7D9', padding: '8px 0' }}>
          <p style={{ textAlign: 'center', fontFamily: "'Helvetica Neue',Arial,sans-serif", fontSize: 10, color: '#94A3B8', textTransform: 'uppercase', letterSpacing: '0.1em', margin: '0 0 6px', fontWeight: 700 }}>Partenaires santé</p>
      ...
      <div style={{ flex: 1, display: 'flex' }}>
        ...
        <div style={{ flex: 1, background: `linear-gradient(135deg, #0D2818 0%, ${primaire} 100%)`, display: 'none', alignItems: 'center', justifyContent: 'center', padding: 48, position: 'relative', overflow: 'hidden' }}
          ...
          <div style={{ position: 'relative', textAlign: 'center', maxWidth: 380 }}>
            ...
            <p style={{ fontFamily: "'Helvetica Neue',Arial,sans-serif", fontSize: 15, color: 'rgba(255,255,255,0.75)', lineHeight: 1.7, margin: '0 0 32px' }}>
              Protocoles cliniques, simulations médicales et certifications reconnues par le MSP Haïti.
            </p>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              {['2 400+ professionnels inscrits', '62 modules de formation', 'Certifications MSP Haïti', 'Protocoles OMS adaptés'].map(item => (
        ...
        <div style={{ width: '100%', maxWidth: 480, margin: '0 auto', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 'clamp(24px,5vw,48px) 24px' }}>
          <div style={{ width: '100%' }}>
            ...
            <div style={{ textAlign: 'center', marginBottom: 32 }}>
              ...
              <h1 style={{ fontFamily: 'Georgia,serif', fontSize: 24, fontWeight: 'normal', color: '#0D2818', margin: '0 0 6px' }}>MediForm Haiti</h1>
              <p style={{ fontFamily: "'Helvetica Neue',Arial,sans-serif", fontSize: 13, color: '#4A7A5A', margin: 0 }}>Plateforme médicale & paramédicale haïtienne</p>
            </div>
            ...
            <div style={{ background: 'white', borderRadius: 20, padding: 'clamp(24px,4vw,36px)', border: '1px solid #D1E7D9', boxShadow: `0 8px 40px ${primaire}10` }}>
              ...
              <h2 style={{ fontFamily: 'Georgia,serif', fontSize: 22, fontWeight: 'normal', color: '#0D2818', margin: '0 0 6px', textAlign: 'center' }}>Connexion</h2>
              <p style={{ fontFamily: "'Helvetica Neue',Arial,sans-serif", fontSize: 13, color: '#4A7A5A', textAlign: 'center', margin: '0 0 24px' }}>
              ...
              <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                <div>
                  ...
                  <input type="email" value={email} required onChange={e => setEmail(e.target.value)}
                    placeholder="medecin@exemple.ht" style={inp}
                    onFocus={e => { e.target.style.borderColor = primaire; e.target.style.boxShadow = `0 0 0 3px ${primaire}15`; }}
                    onBlur={e => { e.target.style.borderColor = '#D1E7D9'; e.target.style.boxShadow = 'none'; }} />
                </div>
                <div>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
                    <label style={{ fontFamily: "'Helvetica Neue',Arial,sans-serif", fontSize: 13, fontWeight: 600, color: '#0D2818' }}>Mot de passe</label>
                  ...
                  <div style={{ position: 'relative' }}>
                    ...
                    <button type="button" onClick={() => setVoirMDP(v => !v)}
                      style={{ position: 'absolute', right: 14, top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', cursor: 'pointer', fontSize: 16, color: '#6B9E7A' }}>
                      {voirMDP ? '🙈' : '👁'}
                    </button>
                  </div>
                </div>
                ...
                <button type="submit" disabled={chargement}
                  style={{ width: '100%', padding: '15px', background: chargement ? '#4A7A5A' : primaire, color: 'white', border: 'none', borderRadius: 12, fontFamily: "'Helvetica Neue',Arial,sans-serif", fontWeight: 700, fontSize: 16, cursor: chargement ? 'not-allowed' : 'pointer', transition: 'background 0.2s', marginTop: 4 }}>
            ...
            <p style={{ fontFamily: "'Helvetica Neue',Arial,sans-serif", fontSize: 11, color: '#6B9E7A', textAlign: 'center', marginTop: 16 }}>
              🔒 Connexion sécurisée · Certifications MSP Haïti
            </p>
          </div>
        </div>
      </div>
